...
if ($_GET['id']!=''&&$_GET['newname']!='') {
	...
	if (mysql_query($sql_update_name)) {
		echo "success";
	}else{
		echo "filed";
	}
...
if ($_GET['new_dir_key']!='') {
	...
	$sql_get_dirs_temp_arr = mysql_query($sql_get_dirs_temp);
	$arr = array();
	while ($row = mysql_fetch_assoc($sql_get_dirs_temp_arr)) {
		$arr[] = $row;
	}
	$end = array(
		'result' => $arr
...
if ($_GET['get_list']!='') {
	...
	$sql_get_dirs_temp_arr = mysql_query($sql_get_dirs_temp);
	$sql_get_files_temp_arr = mysql_query($sql_get_files_temp);
	$dirs_arr = array();
	$files_arr = array();
	while ($row = mysql_fetch_assoc($sql_get_dirs_temp_arr)) {
		$dirs_arr[] = $row;
	}
	while ($row = mysql_fetch_assoc($sql_get_files_temp_arr)) {
		$files_arr[] = $row;
	}
	$end = array(
		'dirs' => $dirs_arr,
