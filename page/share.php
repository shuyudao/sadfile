...
$sql_share = mysqli_query($conn,$sql_select_share);
$data_share = array();
while ($row = mysqli_fetch_assoc($sql_share)) {
	$file_id = $row['file_id']; #获取到文件ID，从而获得文件信息
	$sql_select_file = "SELECT * FROM files WHERE id = $file_id";
	$temp_file = mysqli_fetch_assoc(mysqli_query($conn,$sql_select_file));#获得文件信息
	//添加——> 数组
								...
								<?php for($i = 0 ; $i < count($data_share) ; $i++){
								 ...
								 ?>
							        <tr>
							            ...
							            <td><a target="_blank" href="<?php echo $data[0]['site_url'].'/page/fx.php?key='.$v['share_key']?>"><span class="am-badge am-badge-primary am-icon-link"> 点击</span></a></td>
